...
const LibraryDetails = ({route}) => {
  ...
  useEffect(() => {
    const fetchClearanceData = async () => {
      try {
        const response = await fetch(BASE_URL + `library.php?regNo=${regNo}`);
        if (!response.ok) {
      ...
      {clearanceData && (
        <View style={styles.container}>
          <Text style={{color: 'blue', textAlign: 'center', fontSize: 23}}>
            Library Clearance status for {regNo}:
          </Text>
          ...
          <View style={styles.flexis}>
            <Text>Book Issued:</Text>
            <Text>{clearanceData[0].book_name}</Text>
          </View>
          <View style={styles.flexis}>
            <Text>Enrolled Year:</Text>
          ...
          <View style={{flexDirection: 'row', gap: 10}}>
            ...
            <Text style={{color: 'gray', padding: 10}}>
              Endeavour to meet the bursar department for library clarification
